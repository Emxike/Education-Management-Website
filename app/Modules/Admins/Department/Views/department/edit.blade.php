...
@section("content")
    <div class="card shadow mb-4">
        <div class="card-body">
            <div class="bgc-white p-20 bd"><h3 class="c-grey-900">Update Faculty</h3>
                <div class="mT-30">
                    <form action="{{ route("department.update", ["id" => $data->id]) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                ...
                                <input type="text" class="form-control" value="{{ old("department_name", $data->department_name) }}"
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <select id="manager" class="form-control" name="manager">
                                    ...
                                    @foreach($users as $user)
                                        <option value="{{ $user->id }}" @if(old("manager", $data->member_id) == $user->id) selected="selected" @endif>{{ $user->user_name . '-' . $user->full_name }}</option>
                                    @endforeach
                                </select>
                                @error('manager')
                                <span class="text-danger">{{ $message }}</span>
